...
let gameStartState = {
  ...
  gold: 10,
  testingMode: false,
  doubleEndOfTurnVillagers: false,
  ...
  fightDevGiftCount: 0,
  fightDevGiftTotal: 0,
  fightDevDrainCount: 0,
  fightDevDrainTotal: 0,
  selfDamageBlock: 0,
  devGiftBlock: 0,
  devGiftAttack: 0,
  selfDamageAttack: 0,
...
function createRandomMonster() {
  let monsterArray = Object.values(newTestOpponentMonsters)
  let randomValues = [-1, 0, 1]
  let randomI = Math.floor(Math.random() * monsterArray.length)
  let randomMonster = monsterArray[randomI]
  let randomHPChange = randomValues[Math.floor(Math.random() * randomValues.length)]
  console.log("random I is " + randomI)
  console.log("mosnter array is  " + monsterArray)
  console.log("chosen monster is  " + monsterArray[randomI])
  randomMonster.maxHP += randomHPChange
  randomMonster.currentHP += randomHPChange
  randomMonster.baseDamage += randomValues[Math.floor(Math.random() * randomValues.length)] 
...
async function loseDevelopment(stateObj, devToLose, targetIndex=0, playerTriggered=false) {
  if (devToLose <= 0 || stateObj.opponentMonster[targetIndex].development === 0) {
    return stateObj
  }
  if (devToLose > 0 && stateObj.opponentMonster[targetIndex].development < devToLose) {
  ...
  if (playerTriggered === true) {
    stateObj = immer.produce(stateObj, (newState) => {
      ...
      newState.fightDevDrainTotal += devToLose
    })
    await pause(350)
  }
  stateObj = immer.produce(stateObj, (newState) => {
...
async function opponentLoseEnergy(stateObj, energyToLose, targetIndex=0, playerTriggered=false) {
  return loseDevelopment(stateObj, energyToLose, targetIndex, playerTriggered);
}
...
async function gainDevelopment(stateObj, devToGain, targetIndex=0, playerTriggered=false) {
  ...
  if (currentDev + devToGain > maxDev) {
  ...
  if (playerTriggered === true) {
    stateObj = immer.produce(stateObj, (newState) => {
      ...
      newState.fightDevGiftTotal += devToGain
    })
    await pause(350)
  }
...
async function opponentGainEnergy(stateObj, energyToGain, targetIndex=0, playerTriggered=false) {
  return gainDevelopment(stateObj, energyToGain, targetIndex, playerTriggered);
}
...
async function energyGift(stateObj, devToGain, villagerCost=false, all=false) {

  stateObj = await gainDevelopment(stateObj, devToGain, stateObj.targetedMonster, playerTriggered=true)
...
async function destroyEnergy(stateObj, devToDestroy, villagerCost=false, all=false) {
    if (all === true) {
      stateObj.opponentMonster.forEach(async function (monsterObj, monsterIndex) {
        if (monsterObj.development > 0 && monsterObj.development > devToDestroy)  {
          stateObj = await loseDevelopment(stateObj, devToDestroy, monsterIndex, playerTriggered=true)
        } else if (monsterObj.development > 0) {
          devToDestroy = monsterObj.development
          stateObj = await loseDevelopment(stateObj, devToDestroy, monsterIndex, playerTriggered=true)
        } else {}
      })
    } else {
      ...
      if (monsterObj.development > 0 && monsterObj.development > devToDestroy)  {
        stateObj = await loseDevelopment(stateObj, devToDestroy, stateObj.targetedMonster, playerTriggered=true)
      } else if (monsterObj.development > 0) {
        devToDestroy = monsterObj.development
        stateObj = await loseDevelopment(stateObj, devToDestroy, stateObj.targetedMonster, playerTriggered=true)
      } else {}
...
async function gainDiplomacy(stateObj, amount) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState.diplomacy += amount;
  });
  // Check for diplomacy win
  if (stateObj.diplomacy >= stateObj.diplomacyThreshold) {
    // Increment threshold for next time (only diplomacy wins do this)
    stateObj = immer.produce(stateObj, (newState) => {
      newState.diplomacyThreshold += 1;
      // Kill all enemies to trigger normal win flow
      newState.opponentMonster.forEach(function(monsterObj) {
        monsterObj.currentHP = 0;
      });
    });
  }
  return stateObj;
...
// Apply treason to the targeted enemy
async function applyTreason(stateObj, amount, targetIndex) {
  if (typeof targetIndex === 'undefined') {
    targetIndex = stateObj.targetedMonster || 0;
  }
  // Brief dark flash on the target
  let monsterAvatar = document.querySelectorAll("#opponents .avatar")[targetIndex];
  if (monsterAvatar) {
    monsterAvatar.classList.add('dark-filter');
  }
  await pause(300);
  stateObj = immer.produce(stateObj, (newState) => {
    if (newState.opponentMonster[targetIndex]) {
      newState.opponentMonster[targetIndex].treason = (newState.opponentMonster[targetIndex].treason || 0) + amount;
    }
  });
  if (monsterAvatar) {
...
// Get effective tribute cost after discount (minimum 0)
function getEffectiveTribute(stateObj, tributeAmount) {
  return Math.max(0, tributeAmount - (stateObj.tributeDiscount || 0));
}

// Pay tribute (subtract gold). Returns updated state.
async function payTribute(stateObj, tributeAmount) {
  let effectiveCost = getEffectiveTribute(stateObj, tributeAmount);
  // Gold burst animation
  let goldDiv = document.getElementById("goldDiv");
  let goldText = document.getElementById("goldText");
  if (goldDiv) {
    goldDiv.classList.add('tribute-burst');
  }
  if (goldText) {
    goldText.classList.add('tribute-shrink');
  }
  await pause(200);
  stateObj = immer.produce(stateObj, (newState) => {
    newState.gold -= effectiveCost;
  });
  // Update the displayed number mid-animation
  if (goldText) {
    goldText.textContent = stateObj.gold;
  }
  await pause(500);
  if (goldDiv) {
    goldDiv.classList.remove('tribute-burst');
  }
  if (goldText) {
    goldText.classList.remove('tribute-shrink');
  }
  return stateObj;
}

// Check if player can afford tribute
function canAffordTribute(stateObj, tributeAmount) {
  let effectiveCost = getEffectiveTribute(stateObj, tributeAmount);
  return stateObj.gold >= effectiveCost;
}
...
// Select a player structure (for building and card effects)
function selectPlayerStructure(stateObj, structureIndex) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState.selectedPlayerStructure = structureIndex;
  });
  changeState(stateObj);
  return stateObj;
}

// Deal damage to a specific structure (block-then-HP)
function dealStructureDamage(stateObj, damage, side, structureIndex) {
  stateObj = immer.produce(stateObj, (newState) => {
    let arrayKey = (side === "player") ? "playerStructures" : "opponentStructures";
    let structure = newState[arrayKey][structureIndex];
    if (!structure) return;

    if (structure.encounterBlock === 0) {
      structure.currentHP -= damage;
    } else if (structure.encounterBlock >= damage) {
      structure.encounterBlock -= damage;
    } else {
      let remainder = damage - structure.encounterBlock;
      structure.encounterBlock = 0;
      structure.currentHP -= remainder;
    }
  });
  return stateObj;
}
...
// Remove dead structures (no XP/gold) — only enemy structures can die
async function handleStructureDeaths(stateObj) {
  stateObj = immer.produce(stateObj, (newState) => {
    // Player structures don't have HP, never die
    newState.opponentStructures = newState.opponentStructures.filter(s => s.currentHP > 0);
  });
  return stateObj;
}

// Target a specific structure (for structure-targeting cards)
function targetThisStructure(stateObj, side, structureIndex) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState.targetedStructure = { side: side, index: structureIndex };
  });
  changeState(stateObj);
  return stateObj;
}

// Deal damage to the currently targeted structure
async function dealTargetedStructureDamage(stateObj, damage) {
  if (stateObj.targetedStructure && stateObj.targetedStructure.side !== null) {
    stateObj = dealStructureDamage(stateObj, damage, stateObj.targetedStructure.side, stateObj.targetedStructure.index);
    stateObj = await handleStructureDeaths(stateObj);
  }
  return stateObj;
}
...
function resetAfterFight(stateObj) {
  ...
  stateObj = immer.produce(stateObj, (newState) => {
    ...
    newState.fightDevDrainCount = 0;
    newState.fightDevDrainTotal = 0;
    newState.fightDevGiftCount = 0;
    newState.fightDevGiftTotal = 0;
...
async function drawAHand(stateObj) {
  ...
  let cardsToDraw = 5 - 1; // draw 5 cards (loop is inclusive of 0)
...
function topRowDiv(stateObj, divName) {
  ...
  if (stateObj.doubleEndOfTurnVillagers === true) {
    let extraHealDiv = document.createElement("Div");
...
function doubleEndEnergy(stateObj) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState.playerMonster.startingEnergy -= 1;
    newState.doubleEndOfTurnVillagers = true;
    newState.status = Status.OverworldMap
    newState.townMapSquares[newState.playerHere] = "completed"
  })
  changeState(stateObj);
...
function renderCard(stateObj, cardArray, index, divName=false, functionToAdd=false, goldCost=false) {
  ...
  let cardDiv = document.createElement("Div");
        ...
        //if cardArray is the hand, add playable class to the cards if energy > card.minReq and tribute is affordable
        if (cardArray === stateObj.encounterHand) {
          let hasEnergy = false;
          if (typeof cardObj.minReq === 'function') {
            hasEnergy = cardObj.minReq(stateObj, index, stateObj.encounterHand) <= stateObj.playerMonster.encounterEnergy;
          } else {
            hasEnergy = cardObj.minReq <= stateObj.playerMonster.encounterEnergy;
          }
          let canAfford = cardObj.tribute ? canAffordTribute(stateObj, cardObj.tribute) : true;
          if (hasEnergy && canAfford) {
            cardDiv.classList.add("playable");
...
function renderStructures(stateObj, side) {
  ...
  structures.forEach(function(structureObj, index) {
    ...
    if (side === "player") {
      ...
      hpBarOuter.classList.add("structure-hp-bar-outer");
      let hpBarFill = document.createElement("Div");
      hpBarFill.classList.add("structure-hp-bar-fill");
      let hpPercent = Math.max(0, (structureObj.currentHP / structureObj.maxHP) * 100);
      hpBarFill.style.width = hpPercent + "%";
      hpBarOuter.appendChild(hpBarFill);
      structDiv.appendChild(hpBarOuter);

      // HP text
      let hpText = document.createElement("span");
      hpText.classList.add("structure-hp-text");
      hpText.textContent = structureObj.currentHP + "/" + structureObj.maxHP;
      structDiv.appendChild(hpText);

      // Block display
      if (structureObj.encounterBlock > 0) {
        let blockDiv = document.createElement("span");
        blockDiv.classList.add("structure-block");
        blockDiv.textContent = " [" + structureObj.encounterBlock + " fort]";
        structDiv.appendChild(blockDiv);
      }
      ...
      // Click to target (for cards that target structures)
      if (stateObj.targetedStructure && stateObj.targetedStructure.side === side && stateObj.targetedStructure.index === index) {
        structDiv.classList.add("structure-targeted");
      } else {
        structDiv.classList.add("structure-clickable");
        structDiv.addEventListener("click", function() {
...
function renderOpponents(stateObj) {
  ...
  stateObj.opponentMonster.forEach(function (monsterObj, index) {
    ...
    if (monsterObj.treason && monsterObj.treason > 0) {
      let treasonDiv = document.createElement("Div");
      treasonDiv.classList.add("treason");
      let treasonTextSpan = document.createElement("span");
    ...
    monsterObj.moves.forEach(function(moveObj, moveIndex) {
      ...
      let devReq = moveObj.devRequirement || 0;
      ...
      if (isActive) {
        ...
        moveDiv.classList.add("move-locked");
      } else {
...
// Development-gated move selection: pick the highest-tier move the enemy can afford
async function pickOpponentMove(stateObj) {
  return immer.produce(stateObj, (newState) => {
    newState.opponentMonster.forEach(function (monsterObj, index) {
      let chosenIndex = 0;
      for (let i = 0; i < monsterObj.moves.length; i++) {
        if (monsterObj.moves[i].name && monsterObj.development >= (monsterObj.moves[i].devRequirement || 0)) {
          chosenIndex = i;
        }
      }
      newState.opponentMonster[index].opponentMoveIndex = chosenIndex;
    });
  });
}
...
async function endTurn(stateObj) {
  ...
  stateObj = immer.produce(stateObj, (newState) => {
    ...
    newState.opponentStructures.forEach(function (structureObj) {
      structureObj.encounterBlock = 0;
    })
  });
  ...
  // Player structures fire (player's end of turn)
  stateObj = await fireStructureEffects(stateObj, "player");
  stateObj = await handleStructureDeaths(stateObj);
  if (stateObj.opponentMonster.length === 0) {
    return stateObj
  }

  // Enemy takes their turn (may build structures)
  stateObj = await playOpponentMove(stateObj);

  // Enemy structures fire (enemy's end of turn)
  stateObj = await fireStructureEffects(stateObj, "opponent");
  stateObj = await handleStructureDeaths(stateObj);
